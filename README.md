...
```bash
sudo nix --extra-experimental-features "nix-command flakes" run github:nix-community/disko -- --mode disko --flake github:Aliikay/slipstream-testbed-config#slipstream-testbed && sudo mkdir /mnt/temp-install && TMPDIR=/mnt/temp-install sudo nixos-install --flake github:Aliikay/slipstream-testbed-config#slipstream-testbed --show-trace && nixos-enter --root /mnt -c 'passwd slipstream-testbed'
```
